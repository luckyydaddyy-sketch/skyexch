...
function AUserMessage() {
  ...
  const updateSiteData = async () => {
    let data = {
      ...
      value: {
        id: siteData?._id,
        titleUserMessage: siteData?.titleUserMessage,
        userMessage: siteData?.userMessage,
        userMessageDate: startDate,
      },
    }
  ...
  const changeMessage = (e: any) => {
    ...
    if (name === 'title') {
      siteData.titleUserMessage = value
    }
    if (name === 'message') {
      siteData.userMessage = value
    }
      ...
      <div className="container main_wrap" >
        <div className='my-account-section-content'>
          <div className='my-account-section_header'>
            <h4 className="s_mb-8"> Set Message For Users</h4>
            <div className="account_tabs_filter" style={{ marginLeft: "0px" }}>
              <h4> Select Website  </h4>
              <div className="flex-align">
                <div className="input_group s_mr-20 flex-align">
              ...
              <div className="flex-align s_my-10">
                <div className="mb-2">
                  <span>
                    <input type="text" placeholder="Title" name="title" maxLength={16} className="form-control" onChange={(e) => changeMessage(e)} value={siteData?.titleUserMessage} />
                  </span>
                </div>
                <div className="input_group from-to s_ml-10">
                  <div className="no-wrap flex-align">
                    <div className='w_80'>
                      <ReactDatePicker selected={siteData?.userMessageDate ? new Date(siteData?.userMessageDate) : startDate} className='form-control hasDatepicker' onChange={(date: Date) => setStartDate(date)} />
                    </div>
                  </div>
                </div>
              </div>
              <div style={{ width: "30%" }}>
                <textarea style={{ width: "100%", minHeight: "100px" }} name="message" value={siteData?.userMessage} onChange={(e) => changeMessage(e)} />
              </div>
            ...
            <table id="resultTable" className="table01 margin-table">
              ...
              <tbody>
                  ...
                  siteListData && siteListData?.length && siteListData?.map((item: any, i: number) => {
                    return (
                      <tr>
                        ...
                        <td>{item?.domain}</td>
                        <td>{item?._id}</td>
                        <td>{item?.titleUserMessage}</td>
                        <td>{item?.userMessageDate}</td>
                        <td>{item?.status}</td>
                        <td>
                          <div className="search-btn">
                ...
                {/* <tr>
                                    ...
                                    <td>
                                        <div className="search-btn">
                                            <button style={{ ...styleObjectBlackButton(DD?.colorSchema, true) }}>Edit</button>
                                        </div>
                                    </td>
                                </tr> */}
