...
function SessionHistory() {
  ...
  const getPageData = async (TYPE: string, PAGE: string, SEARCH: string = '') => {
    let data: dataInterface = {
      api: ADMIN_API.SETTING.FANCY_HISTORY.GET_LIST,
      value: {
        type: TYPE,
        page: PAGE ? PAGE : '1',
        limit: '50'
    ...
    <div className="container sportleague settings  main_wrap" >
      <div className='top_header'>
        <div className='top_header_title mt-3'>
          <h6 className="font-weight-bold">Manage {tab} Session <strong>History</strong></h6>
        </div>
      </div>
      <div className="tabs">
                  ...
                  <tr>
                    ...
                    <td>
                      <a onMouseEnter={() => handleMouseEnter(item.name)}
                        ...
                        href={'/session/bets/history/' + item._id + '/rollback'}
                        className="btn btn-outline-secondary btn-sm btn_black rollback-result shadow-none">Manage Fancy</a>
                      {/* btn btn-outline-danger btn-sm rollback-result shadow-none */}
                    </td>
                  </tr>
                )
              }) : <><h2>no data</h2></>}
