...
const SkyPopup = (props: any) => {
  ...
  return (
    <>
      <div className={`popup-wrp fade ${customClass ? customClass : ''} ${OpenModal ? "popup-show " : ""}`}>
        <div className="pop" style={maxWidth && {maxWidth: maxWidth}}>
          <div className="pop-content">
            ...
            <form onSubmit={(e) => submit(e, type ? type : '')}>
              ...
              {!isHideSubmit && <div className="pop-footer">
                <div className={`button-wrap pb-0 ${closebtn ? "d_flex d_flex_justify_spacebt d_flex_align_center w_100 " : ""}`}>
                  ...
                  <button onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} style={styleObjectBlackButton(DD?.colorSchema, true)} onClick={(e) => submit(e, type ? type : '')} className="submit-btn btn btn_black" > {props?.btnName ? props?.btnName : (type === 'status' ? "Change" : "Submit")} </button>
                </div>
              </div>}
            </form>
          </div>
