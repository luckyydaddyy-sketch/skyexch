...
async function handler({
    ...
    let UserInfo = await mongo.bettingApp.model(mongo.models.users).findOne({ // Find admin data
        query: {
            _id: userId
        }
    })
    ...
    let updateRes = await mongo.bettingApp.model(mongo.models.users).updateOne({ // update admin data
        query: {
            _id: userId
        },
        update: {
