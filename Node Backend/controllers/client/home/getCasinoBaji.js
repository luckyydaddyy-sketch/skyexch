...
const payload = {
  query: joi.object().keys({
    gameType: joi
      .string()
      .required()
      .allow("Egame", "Fishing", "Casino", "Slot", "Table", "Popular", "Arcade", "Lottery"),
    filter: joi.string().required().allow("catalog", "latest", "a-z"),
    subName: joi.string().optional().allow(""),
    domain: joi.string().optional(),
    platform: joi.array().items(joi.string().optional().allow("")),
...
async function handler({ query }) {
  ...
  if(query?.search && query?.search !== ""){
    queryDb.title = { $regex: `^${query?.search}$`, $options: "i" }
  }
  if(query?.platform && query?.platform.length > 0){
    if (query?.platform[0] !== "All") {
      queryDb.platform = { $in : query?.platform.map(getPlatformName) }
    }
  }
