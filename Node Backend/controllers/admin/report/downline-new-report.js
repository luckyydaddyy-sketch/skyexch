...
async function getUserReport(userId, commission, to, from, filter) {
  ...
  console.log("getUserReport :: to :", to);
  console.log("getUserReport :: from :", from);
  ...
  // const betsCounts = await mongo.bettingApp
  //   .model(mongo.models.betsHistory)
  //   .countDocuments({
  //     query,
  //   });
  // const casinoBetsCounts = await mongo.bettingApp
  //   .model(mongo.models.casinoMatchHistory)
  //   .countDocuments({
  //     query,
  //   });
  // if (betsCounts === 0 && casinoBetsCounts === 0) {
  //   const report = {
  //     stack: 0,
  //     playerProfitLost: 0,
  //     comm: 0,
  ...
  //   return report;
  // }
  const report = await getTotalLostWinForUser(userId, to, from, filter);
...
async function getUserReportForAdmin(userId, commission, to, from, filter) {
  ...
  console.log("getUserReportForAdmin :: to :", to);
  console.log("getUserReportForAdmin :: from :", from);
  // if (filter && filter === "date" && to && from) {
  let tempStartDate = new Date();
  let tempEndDate = new Date();
  if (to && from) {
    const { endDate, startDate } = getStartEndDateTime(from, to);
    console.log("getUserReportForAdmin :getStartEndDateTime: startDate :", startDate);
    console.log("getUserReportForAdmin :getStartEndDateTime: endDate :", endDate);
    query.createdAt = {
      $gte: startDate,
      $lte: endDate,
    };
  ...
  if (
    ...
    const { endDate, startDate } = getDate("today", true);
    console.log("getUserReportForAdmin : today: endDate: ", endDate);
    console.log("getUserReportForAdmin : today: startDate: ", startDate);
    ...
    console.log("myPl:: sport: BET : report: ", report.upLineProfitLost);
    const casinoBetsInfo = await mongo.bettingApp
      .model(mongo.models.casinoMatchHistory)
      .find({
        query: casinoQuery,
    ...
    console.log("casinoBetsInfo.leg :111: ", casinoBetsInfo.length);
    ...
    for await (const casinoBet of casinoBetsInfo) {
      ...
      if (casinoBet.gameStatus === GAME_STATUS.WIN) {
        // report.playerProfitLost -= casinoBet.winLostAmount;
        report.casinoProfitLost -= casinoBet.winLostAmount;
        report.upLineProfitLost -= casinoBet.winLostAmount;
      } else if (casinoBet.gameStatus === GAME_STATUS.LOSE) {
        // report.playerProfitLost += casinoBet.winLostAmount;
  ...
  console.log("Total Report Admin: ", report);
  return report;
}
  ...
  if (adminInfo.agent_level === USER_LEVEL_NEW.M) {
    ...
    for await (const agent of agentInfo) {
      ...
      const userInfoAgent = await mongo.bettingApp
        ...
        console.log("downline report userInfoAgent :: ", userInfoAgent.length);
