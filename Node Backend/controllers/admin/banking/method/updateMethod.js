...
async function handler({ body, user }) {
  const { type, name, accountNo, ifscCode, holderName, id, isActive, paymentType } = body;

  await mongo.bettingApp.model(mongo.models.bankDetails).updateOne({
    query: { _id: id },
    update: {
      type,
      name,
  ...
  const data = { msg: "updated method successfully!" };
